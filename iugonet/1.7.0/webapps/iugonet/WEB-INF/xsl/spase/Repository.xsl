...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:import href="./common/AccessURL.xsl"/>
  ...
  <xsl:template name="Repository">
    ...
    <xsl:sort select="./@metadata_value_id" data-type="number" order="ascending"/>
      ...
      <tr>
        <td class="metadataFieldValue1"><a><xsl:attribute name="href"><xsl:value-of select="concat('/iugonet/browse?type=RepositoryID&amp;value=', .)"/></xsl:attribute><xsl:value-of select="."/></a></td>
      </tr>
    </xsl:for-each>
    ...
    <xsl:call-template name="ResourceHeader">
      ...
      <xsl:with-param name="resource_type">Repository</xsl:with-param>
